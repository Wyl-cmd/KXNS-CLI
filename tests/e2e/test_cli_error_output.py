...
def _run_kxns(args: list[str], *, share_dir: Path) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["KXNS_SHARE_DIR"] = str(share_dir)
    ...
    cmd = [sys.executable, "-m", "kxns_cli.cli", *args]
...
def test_config_option_requires_argument_is_reported(tmp_path: Path) -> None:
    ...
    result = _run_kxns(["--config"], share_dir=share_dir)
...
def test_config_option_help_value_is_reported(tmp_path: Path) -> None:
    share_dir = tmp_path / "share"
    result = _run_kxns(["--config", "--help"], share_dir=share_dir)
    assert result.returncode == snapshot(2)
    assert result.stdout == snapshot("")
    assert _normalize_cli_error_output(result.stderr) == snapshot(
        """\
Usage: python -m kxns_cli.cli [OPTIONS] COMMAND [ARGS]...
Try 'python -m kxns_cli.cli -h' for help.
Error:
...
def test_invalid_config_toml_is_reported(tmp_path: Path) -> None:
    ...
    log_path = share_dir / "logs" / "kxns.log"
