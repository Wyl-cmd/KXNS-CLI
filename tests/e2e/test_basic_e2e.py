...
def _print_trace(label: str, text: str) -> None:
    if os.getenv("KXNS_TEST_TRACE") == "1":
        print("-----")
        print(f"{label}: {text}")
...
async def test_scripted_echo_kxns_cli_agent_e2e(
    ...
    trace_env = os.getenv("KXNS_SCRIPTED_ECHO_TRACE", "0")
    config_data = {
        ...
        "providers": {
            "scripted_provider": {
                ...
                "api_key": "",
                "env": {
                    "KXNS_SCRIPTED_ECHO_SCRIPTS": str(scripts_path),
                    "KXNS_SCRIPTED_ECHO_TRACE": trace_env,
                },
            }
        },
    }
