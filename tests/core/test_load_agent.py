...
from kxns_cli.exception import InvalidToolError, SystemPromptTemplateError
from kxns_cli.session import Session
from kxns_cli.session_state import DynamicSubagentSpec
from kxns_cli.soul.agent import BuiltinSystemPromptArgs, Runtime, _load_system_prompt, load_agent
...
def test_load_system_prompt(system_prompt_file: Path, builtin_args: BuiltinSystemPromptArgs):
    ...
    assert builtin_args.KXNS_NOW in prompt
...
def test_load_system_prompt_allows_literal_dollar(builtin_args: BuiltinSystemPromptArgs):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        system_md.write_text("Price is $100, path $PATH, time ${KXNS_NOW}.")
...
def test_load_tools_valid(runtime: Runtime):
    ...
    tool_paths = ["kxns_cli.tools.think:Think", "kxns_cli.tools.shell:Shell"]
    toolset = KxnsToolset()
    toolset.load_tools(
        tool_paths,
        {
...
def test_load_tools_invalid(runtime: Runtime):
    ...
    tool_paths = ["kxns_cli.tools.nonexistent:Tool", "kxns_cli.tools.think:Think"]
    toolset = KxnsToolset()
    try:
        toolset.load_tools(
            tool_paths,
        ...
        assert "kxns_cli.tools.nonexistent:Tool" in str(e)
...
agent:
  name: "Test Agent"
  system_prompt_path: ./system.md
  tools: ["kxns_cli.tools.nonexistent:Tool"]
""")
...
def system_prompt_file() -> Generator[Path, Any, Any]:
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        system_md = tmpdir / "system.md"
        system_md.write_text("Test system prompt with ${KXNS_NOW} and ${CUSTOM_ARG}")
