...
from kxns_cli.tools.web.fetch import FetchURL, Params
...
async def test_fetch_url_404_url(fetch_url_tool: FetchURL) -> None:
    ...
    assert result.message == snapshot(
        "Failed to fetch URL due to network error: Cannot connect to host github.com:443 ssl:default [信号灯超时时间已到]. This may indicate the URL is invalid or the server is unreachable."
    )
...
async def test_fetch_url_with_service(runtime) -> None:
    ...
    from kxns_cli.config import Config, WebFetchConfig, Services
