...
from kxns_cli.ui.shell.slash import ShellSlashCmdFunc, shell_mode_registry
...
def isolated_share_dir(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> Path:
    ...
    monkeypatch.setattr("kxns_cli.share.get_share_dir", _get_share_dir)
    monkeypatch.setattr("kxns_cli.metadata.get_share_dir", _get_share_dir)
    return share_dir
...
def mock_shell(work_dir: KaosPath) -> Mock:
    """Create a mock Shell whose soul passes the KxnsSoul isinstance check.
...
class TestNewCommandBehavior:
    ...
    async def test_returns_early_without_kxns_soul(self) -> None:
        """When soul is not a KxnsSoul, the command should silently return."""
        shell = Mock()
        shell.soul = Mock()  # plain Mock, not spec=KxnsSoul
