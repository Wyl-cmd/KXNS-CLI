...
RG_VERSION = "15.0.0"
RG_BASE_URL = "https://github.com/Wyl-cmd/kxns-cli/releases/download/binaries/rg"
...
def _find_existing_rg(bin_name: str) -> Path | None:
    ...
    assert kxns_cli.__file__ is not None
    local_dep = Path(kxns_cli.__file__).parent / "deps" / "bin" / bin_name
...
async def _download_and_install_rg(bin_name: str) -> Path:
    ...
    async with new_client_session() as session:
        with tempfile.TemporaryDirectory(prefix="kxns-rg-") as tmpdir:
