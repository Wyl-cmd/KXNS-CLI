...
from kxns_cli.skill import Skill, discover_skills_from_roots, index_skills, resolve_skills_roots
from kxns_cli.soul.approval import Approval, ApprovalState
...
from kxns_cli.utils.path import list_directory
...
class Runtime:
    ...
    async def create(
        ...
        return Runtime(
            ...
            session=session,
            builtin_args=BuiltinSystemPromptArgs(
                KXNS_NOW=datetime.now().astimezone().isoformat(),
                KXNS_WORK_DIR=session.work_dir,
                KXNS_WORK_DIR_LS=ls_output,
                KXNS_AGENTS_MD=agents_md or "",
                KXNS_SKILLS=skills_formatted or "No skills found.",
                KXNS_ADDITIONAL_DIRS_INFO=additional_dirs_info,
            ),
            denwa_renji=DenwaRenji(),
...
async def load_agent(
    ...
    *,
    ...
    Raises:
        ...
        AgentSpecError(KxnsCLIException, ValueError): When the agent specification is invalid.
        SystemPromptTemplateError(KxnsCLIException, ValueError): When the system prompt template
            is invalid.
