...
import kxns_cli.prompts as prompts
from kxns_cli.soul import wire_send
from kxns_cli.soul.agent import load_agents_md
from kxns_cli.soul.context import Context
from kxns_cli.soul.message import system
from kxns_cli.utils.export import is_sensitive_file
from kxns_cli.utils.path import sanitize_cli_path, shorten_home
from kxns_cli.utils.slashcmd import SlashCommandRegistry
from kxns_cli.wire.types import StatusUpdate, TextPart

if TYPE_CHECKING:
    from kxns_cli.soul.kxnssoul import KxnsSoul

type SoulSlashCmdFunc = Callable[[KxnsSoul, str], None | Awaitable[None]]
"""
A function that runs as a KxnsSoul-level slash command.
...
@registry.command
async def init(soul: KxnsSoul, args: str):
    """Analyze the codebase and generate an `AGENTS.md` file"""
    from kxns_cli.soul.kxnssoul import KxnsSoul

    with tempfile.TemporaryDirectory() as temp_dir:
        tmp_context = Context(file_backend=Path(temp_dir) / "context.jsonl")
        tmp_soul = KxnsSoul(soul.agent, context=tmp_context)
        await tmp_soul.run(prompts.INIT)

    agents_md = await load_agents_md(soul.runtime.builtin_args.KXNS_WORK_DIR)
...
@registry.command
async def compact(soul: KxnsSoul, args: str):
...
async def clear(soul: KxnsSoul, args: str):
    """Clear the context"""
...
@registry.command
async def yolo(soul: KxnsSoul, args: str):
...
@registry.command
async def plan(soul: KxnsSoul, args: str):
    ...
    if subcmd == "on":
        ...
        if soul.plan_mode:
            await soul.set_plan_mode_from_manual(False)
...
async def add_dir(soul: KxnsSoul, args: str):
    """Add a directory to the workspace. Usage: /add-dir <path>. Run without args to list added dirs"""  # noqa: E501
    from kaos.path import KaosPath

    from kxns_cli.utils.path import is_within_directory, list_directory
    ...
    work_dir = soul.runtime.builtin_args.KXNS_WORK_DIR
...
@registry.command
async def export(soul: KxnsSoul, args: str):
    """Export current session context to a markdown file"""
    from kxns_cli.utils.export import perform_export
...
async def import_context(soul: KxnsSoul, args: str):
    """Import context from a file or session ID"""
    from kxns_cli.utils.export import perform_import
