...
def _default_wire_command() -> list[str]:
    argv0 = sys.argv[0]
    if argv0:
        ...
        if (
            ...
            return [str(resolved_path), "--wire"]

    return [sys.executable, "-m", "kxns_cli.cli", "--wire"]


def _default_toad_command() -> list[str]:
    if sys.version_info < (3, 14):
        typer.echo("`kxns term` requires Python 3.14+ because Toad requires it.", err=True)
        raise typer.Exit(code=1)
    if importlib.util.find_spec("toad") is None:
        typer.echo(
            "Toad dependency is missing. Install kxns-cli with Python 3.14+ to use `kxns term`.",
            err=True,
        )
        raise typer.Exit(code=1)
...
def run_term(ctx: typer.Context) -> None:
    ...
    wire_args = _default_wire_command()
    wire_command = shlex.join(wire_args)
    toad_parts = _default_toad_command()
    args = [*toad_parts, "wire", wire_command]
