...
from kxns_cli.session import Session as KxnsSession
from kxns_cli.utils.subprocess_env import get_clean_env
from kxns_cli.web.auth import is_origin_allowed, is_private_ip, verify_token
...
from kxns_cli.web.runner.messages import new_session_status_message, send_history_complete
from kxns_cli.web.runner.process import KxnsRunner
from kxns_cli.web.store.sessions import (
    JointSession,
    SessionMetadata,
...
from kxns_cli.wire.types import is_request
...
def get_runner(req: Request) -> KxnsRunner:
    """Get the KxnsRunner from the FastAPI app state."""
    return req.app.state.runner


def get_runner_ws(ws: WebSocket) -> KxnsRunner:
    """Get the KxnsRunner from the FastAPI app state (for WebSocket routes)."""
...
async def list_sessions(
    runner: KxnsRunner = Depends(get_runner),
    limit: int = 100,
    offset: int = 0,
    q: str | None = None,
...
async def create_session(request: CreateSessionRequest | None = None) -> Session:
    ...
    kxns_cli_session = await KxnsSession.create(work_dir=work_dir)
    context_file = kxns_cli_session.dir / "context.jsonl"
    invalidate_sessions_cache()
    invalidate_work_dirs_cache()
    return Session(
        session_id=UUID(kxns_cli_session.id),
        title=kxns_cli_session.title,
        ...
        session_dir=str(kxns_cli_session.dir),
    )
...
async def upload_session_file(
    ...
    session_dir = session.kxns_cli_session.dir
...
async def get_session_upload_file(
    ...
    uploads_dir = (session.kxns_cli_session.dir / "uploads").resolve()
...
async def get_session_file(
    ...
    work_dir = Path(str(session.kxns_cli_session.work_dir)).resolve()
...
def _update_last_session_id(session: JointSession) -> None:
    ...
    kxns_session = session.kxns_cli_session
    work_dir = kxns_session.work_dir
    ...
    work_dir_meta.last_session_id = kxns_session.id
    save_metadata(metadata)


@router.delete("/{session_id}", summary="Delete a session")
async def delete_session(session_id: UUID, runner: KxnsRunner = Depends(get_runner)) -> None:
    """Delete a session."""
    session = get_editable_session(session_id, runner)
    session_process = runner.get_session(session_id)
    if session_process is not None:
        await session_process.stop()
    wd_meta = session.kxns_cli_session.work_dir_meta
...
async def fork_session(
    ...
    source_dir = source_session.kxns_cli_session.dir
    ...
    work_dir = source_session.kxns_cli_session.work_dir
    new_session = await KxnsSession.create(work_dir=work_dir)
...
async def session_stream(
    session_id: UUID,
    websocket: WebSocket,
    runner: KxnsRunner = Depends(get_runner_ws),
) -> None:
    ...
    try:
        ...
        try:
            ...
            work_dir = Path(str(session.kxns_cli_session.work_dir))
...
def _get_work_dirs_sync() -> list[str]:
    ...
    for wd in metadata.work_dirs:
        ...
        if "/tmp" in wd.path or "/var/folders" in wd.path or "/.cache/" in wd.path or "\\AppData\\Local\\Temp" in wd.path:
            continue
...
async def get_startup_dir(request: Request) -> str:
    """Get the directory where kxns web was started."""
