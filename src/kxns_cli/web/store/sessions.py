...
from kxns_cli.metadata import WorkDirMeta, load_metadata
from kxns_cli.session import Session as KxnsSession
from kxns_cli.web.models import Session
...
class JointSession(Session):
    """Combined session model with both web UI and kxns-cli session data."""

    model_config = ConfigDict(arbitrary_types_allowed=True)

    kxns_cli_session: KxnsSession = Field(exclude=True)
...
def _build_kxns_session(entry: SessionIndexEntry) -> KxnsSession:
    from kaos.path import KaosPath

    from kxns_cli.session_state import load_session_state
...
def _build_joint_session(entry: SessionIndexEntry) -> JointSession:
    kxns_session = _build_kxns_session(entry)
