"""Session process management for Kxns Hunter CLI web interface."""
...
from kxns_cli.web.models import (
    SessionNoticeEvent,
    SessionNoticePayload,
    SessionState,
    SessionStatus,
)
from kxns_cli.web.runner.messages import new_session_status_message
from kxns_cli.web.store.sessions import load_session_by_id
from kxns_cli.wire.jsonrpc import (
...
from kxns_cli.wire.serde import deserialize_wire_message

JSONRPCOutMessageAdapter = TypeAdapter[JSONRPCOutMessage](JSONRPCOutMessage)


class SessionProcess:
    """Manages a single session's KxnsCLI subprocess.

    Handles:
    - Starting/stopping the subprocess
    - Reading from stdout (wire messages from KxnsCLI)
    - Writing to stdin (user input to KxnsCLI)
        ...
        try:
            while True:
                        ...
                        await self._broadcast(
                            JSONRPCErrorResponse(
                                ...
                                error=JSONRPCErrorObject(
                                    code=self._process.returncode if self._process.returncode is not None else -1,
                                    message=stderr_text,
                                ),
                            ).model_dump_json()
                        )
                ...
                try:
                    ...
                    logger.error(f"Failed to parse JSONRPC out message: {e.__class__.__name__} {e}")
    ...
    async def _encode_uploaded_files(self) -> AsyncGenerator[ContentPart]:
        ...
        session = await asyncio.to_thread(load_session_by_id, self.session_id)
        if session is None:
            logger.warning(f"Session {self.session_id} not found for file encoding")
            return

        uploads_dir = session.kxns_cli_session.dir / "uploads"
        if not await asyncio.to_thread(uploads_dir.exists):
            return

        sent_marker = uploads_dir / ".sent"
        if await asyncio.to_thread(sent_marker.exists):
            try:
                sent_text = await asyncio.to_thread(sent_marker.read_text, "utf-8")
                already_sent = json.loads(sent_text)
        ...
        # Persist sent files marker so they won't be re-sent after process restart
        try:
            await asyncio.to_thread(
                lambda: sent_marker.write_text(
                    json.dumps(sorted(self._sent_files)), encoding="utf-8"
                )
            )
        except Exception:
            pass
        ...
        match message:
            case JSONRPCPromptMessage():
                ...
                if isinstance(message.params.user_input, str):
                    if message.params.user_input != "KXNS_FILE_UPLOAD_WITHOUT_MESSAGE":
                ...
                if message.params.thinking is not None:
                    prompt_params["thinking"] = message.params.thinking
                if message.params.model is not None:
                    prompt_params["model"] = message.params.model
                return json.dumps(
                    {
                        "jsonrpc": "2.0",
                        "method": "prompt",
    ...
    async def send_message(self, message: str) -> None:
        ...
        async with self._lock:
            ...
            if process is None or process.stdin is None:
                logger.warning("Cannot send message: process or stdin is None")
                return

        prompt_id_to_rollback: str | None = None
        ...
        try:
            ...
            if isinstance(in_message, JSONRPCPromptMessage):
                ...
                prompt_id_to_rollback = in_message.id
                if not was_busy:
            ...
            logger.error(f"{e.__class__.__name__} {e}: Failed to process JSONRPC in message: {message}")
            if prompt_id_to_rollback is not None:
                self._in_flight_prompt_ids.discard(prompt_id_to_rollback)
                await self._broadcast(
                    JSONRPCErrorResponse(
                        id=prompt_id_to_rollback,
                        error=JSONRPCErrorObject(
                            code=-32000,
                ...
                if not self.is_busy:
                    await self._emit_status("error", reason="process_failed")
            return

        try:
            process.stdin.write((message + "\n").encode("utf-8"))
            await process.stdin.drain()
            prompt_id_to_rollback = None
        except (BrokenPipeError, ConnectionResetError, OSError) as e:
            logger.warning(f"Failed to write to process stdin: {e}")
            if prompt_id_to_rollback is not None:
                self._in_flight_prompt_ids.discard(prompt_id_to_rollback)
                await self._broadcast(
                    JSONRPCErrorResponse(
                        ...
                        error=JSONRPCErrorObject(
                            code=-32000,
                            message=f"Failed to send message to worker: {e}",
                        ),
                    ).model_dump_json()
                )
                if not self.is_busy:
                    await self._emit_status("error", reason="write_failed")
