...
from kxns_cli.auth.platforms import get_platform_name_for_provider, refresh_managed_models
from kxns_cli.cli import Reload, SwitchToWeb
from kxns_cli.config import load_config, save_config
...
from kxns_cli.utils.changelog import CHANGELOG
from kxns_cli.utils.datetime import format_relative_time
from kxns_cli.utils.slashcmd import SlashCommand, SlashCommandRegistry
...
def ensure_kxns_soul(app: Shell) -> KxnsSoul | None:
    if not isinstance(app.soul, KxnsSoul):
        console.print("[red]KxnsSoul required[/red]")
        return None
    return app.soul
...
def help(app: Shell, args: str):
    ...
    from kxns_cli.utils.rich.columns import BulletColumns
...
def version(app: Shell, args: str):
    ...
    console.print(f"kxns, version {VERSION}")


@registry.command
async def model(app: Shell, args: str):
    """Switch LLM model or thinking mode"""
    from kxns_cli.llm import derive_model_capabilities
    ...
    if not config.models:
        console.print('[yellow]No models configured. Use "kxns api <url> <key> <model>" to configure.[/yellow]')
        return
...
async def editor(app: Shell, args: str):
    ...
    from kxns_cli.utils.editor import get_editor_command
...
async def clear(app: Shell, args: str):
    """Clear the context"""
    if ensure_kxns_soul(app) is None:
        return
...
def web(app: Shell, args: str):
    """Open KXNS Hunter Web UI in browser"""
    soul = ensure_kxns_soul(app)
...
async def mcp(app: Shell, args: str):
    ...
    if not isinstance(toolset, KxnsToolset):
        console.print("[red]KxnsToolset required[/red]")
        return
    ...
    for name, info in servers.items():
        ...
        if info.status == "unauthorized":
            server_text += " [grey50](unauthorized - run: kxns mcp auth {name})[/grey50]"
