...
from kxns_cli.ui.shell.prompt import CustomPromptSession, PromptMode, toast
from kxns_cli.ui.shell.replay import replay_recent_history
from kxns_cli.ui.shell.slash import registry as shell_slash_registry
from kxns_cli.ui.shell.slash import shell_mode_registry
from kxns_cli.ui.shell.update import GITHUB_RELEASES_URL, LATEST_VERSION_FILE, UpdateResult, do_update, semver_tuple
from kxns_cli.ui.shell.visualize import visualize
from kxns_cli.utils.envvar import get_env_bool
from kxns_cli.utils.logging import open_original_stderr
from kxns_cli.utils.signals import install_sigint_handler
from kxns_cli.utils.slashcmd import SlashCommand, SlashCommandCall, parse_slash_command_call
from kxns_cli.utils.subprocess_env import get_clean_env
from kxns_cli.utils.term import ensure_new_line, ensure_tty_sane
from kxns_cli.wire.types import ContentPart, StatusUpdate
...
class Shell:
    ...
    async def run(self, command: str | None = None) -> bool:
        ...
        if not get_env_bool("kxns_cli_NO_AUTO_UPDATE"):
            self._start_background_task(self._auto_update())

        _print_welcome_info(self.soul.name or "KXNS Hunter CLI", self._welcome_info)

        if isinstance(self.soul, KxnsSoul):
        ...
        with CustomPromptSession(
            ...
            editor_command_provider=lambda: (
                self.soul.runtime.config.default_editor if isinstance(self.soul, KxnsSoul) else ""
            ),
        ) as prompt_session:
            try:
                while True:
    ...
    async def _run_slash_command(self, command_call: SlashCommandCall) -> None:
        from kxns_cli.cli import Reload, SwitchToWeb
    ...
    async def run_soul_command(self, user_input: str | list[ContentPart]) -> bool:
        ...
        try:
            ...
            await run_soul(
                ...
                cancel_event,
                self.soul.wire_file if isinstance(self.soul, KxnsSoul) else None,
            )
            return True
        except LLMNotSet:
            logger.exception("LLM not set:")
            console.print('[red]LLM not set. Use "kxns api <url> <key> <model>" to configure.[/red]')
    ...
    async def _auto_update(self) -> None:
        ...
        if result == UpdateResult.UPDATE_AVAILABLE:
            while True:
                toast(
                    f"new version found, visit {GITHUB_RELEASES_URL} to upgrade",
                    topic="update",
                    duration=30.0,
                )
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    head = Text.from_markup("[bold red]Kxns Hunter CLI[/bold red] - Penetration Testing AI Agent")
    help_text = Text.from_markup("[grey50]Type /help for available commands. Press Ctrl-X to switch modes.[/grey50]")

    logo = Text.from_markup(_LOGO)

    rows: list[RenderableType] = [logo, Text(""), head, help_text]
        ...
        if semver_tuple(latest_version) > semver_tuple(current_version):
            rows.append(
                Text.from_markup(
                    ...
                    f"Visit {GITHUB_RELEASES_URL} to download.[/yellow]"
                )
            )
