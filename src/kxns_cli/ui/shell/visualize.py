...
from kxns_cli.soul import format_context_status
from kxns_cli.tools import extract_key_argument
from kxns_cli.ui.shell.console import console
from kxns_cli.ui.shell.keyboard import KeyboardListener, KeyEvent
from kxns_cli.utils.aioqueue import QueueShutDown
from kxns_cli.utils.diff import format_unified_diff
from kxns_cli.utils.logging import logger
from kxns_cli.utils.rich.columns import BulletColumns
from kxns_cli.utils.rich.markdown import Markdown
from kxns_cli.utils.rich.syntax import KxnsSyntax
from kxns_cli.wire import WireUISide
    ...
    def _render_todo_markdown(self, block: TodoDisplayBlock) -> str:
        ...
        for todo in block.items:
            ...
            match normalized:
                ...
                case "in progress":
                    lines.append(f"- {todo.title} [IN PROGRESS]")
                case "done":
                    lines.append(f"- ~~{todo.title}~~")
                case _:
                    lines.append(f"- {todo.title}")
...
class _ApprovalRequestPanel:
    def __init__(self, request: ApprovalRequest):
        ...
        self.options: list[tuple[str, ApprovalResponse.Kind]] = [
            ...
            ("Reject, tell Kxns what to do instead", "reject"),
        ]
    ...
    def _render_block(
        ...
        if block.lexer:
            return KxnsSyntax(text, block.lexer)
