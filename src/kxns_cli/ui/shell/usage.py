...
from kxns_cli.auth.platforms import get_platform_by_id, parse_managed_provider_key
from kxns_cli.config import LLMModel
from kxns_cli.soul.kxnssoul import KxnsSoul
from kxns_cli.ui.shell.console import console
from kxns_cli.ui.shell.slash import registry
from kxns_cli.utils.aiohttp import new_client_session
from kxns_cli.utils.datetime import format_duration
...
async def usage(app: Shell, args: str):
    ...
    assert isinstance(app.soul, KxnsSoul)
    ...
    if usage_url is None:
        console.print("[yellow]Usage is available on KXNS Hunter platform only.[/yellow]")
        return
    ...
    with console.status("[cyan]Fetching usage...[/cyan]"):
        ...
        try:
            ...
            if e.status == 401:
                ...
                message = "Usage endpoint not available. Try KXNS For Coding."
...
def _usage_url(model: LLMModel | None) -> str | None:
    ...
    if platform is None or platform.id != KXNS_CODE_PLATFORM_ID:
        return None
