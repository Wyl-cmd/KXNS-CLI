...
KXNS_ANSI_THEME_NAME = "kxns-ansi"
KXNS_ANSI_THEME = ANSISyntaxTheme(
    {
...
def resolve_code_theme(theme: str | SyntaxTheme) -> str | SyntaxTheme:
    if isinstance(theme, str) and theme.lower() == KXNS_ANSI_THEME_NAME:
        return KXNS_ANSI_THEME
    return theme


class KxnsSyntax(Syntax):
    def __init__(self, code: str, lexer: str, **kwargs: Any) -> None:
        if "theme" not in kwargs or kwargs["theme"] is None:
            kwargs["theme"] = KXNS_ANSI_THEME
...
if __name__ == "__main__":
    ...
    for idx, (title, lexer, code) in enumerate(examples):
        ...
        console.print(KxnsSyntax(code, lexer))
