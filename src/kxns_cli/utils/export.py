...
from kxns_cli.utils.message import message_stringify
from kxns_cli.utils.path import sanitize_cli_path
...
async def perform_export(
    ...
    default_name = f"kxns-export-{short_id}-{now.strftime('%Y%m%d-%H%M%S')}.md"
...
async def perform_import(
    ...
    UI output, etc.).
    """
    from kxns_cli.soul.compaction import estimate_text_tokens
