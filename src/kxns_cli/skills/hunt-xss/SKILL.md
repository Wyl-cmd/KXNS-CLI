...
## Advanced Technical Reference (from xss-playbook)
...
When input is inserted via `.innerHTML`, `document.write`, or jQuery `.html()` (not reflected in source):
...
When a page has `window.addEventListener('message', handler)` **without origin validation**:

```html
<iframe src="TARGET_URL" onload="frames[0].postMessage('INJECTION','*')">
```

**Origin bypass:** When origin IS checked but uses `.includes()` or prefix match — attacker registers `facebook.com.attacker.com` and the check passes.

### XML-Based XSS

When the response Content-Type is `text/xml` or `application/xml`:
...
### CSP Bypass: AngularJS CDN

When CSP allows `ajax.googleapis.com`:
...
### CSP Bypass: Angular Expressions (CSTI)

When `{{1+1}}` evaluates to `2` on the page — classic Client-Side Template Injection indicator:
...
When CSP is present but doesn't restrict `base-uri`:
...
Leaks subsequent page content to attacker domain via image request URL.

### WordPress XSS → RCE Chain

When XSS runs in a WordPress admin session — chain through the plugin editor to achieve remote code execution:
...
6. **Key insight**: Filter presence = vulnerability exists, developer tried to patch. Chase that thread across the entire application. If they filter `<script>` in search, check if they filter it in file upload filenames, profile bio, and other input vectors.
