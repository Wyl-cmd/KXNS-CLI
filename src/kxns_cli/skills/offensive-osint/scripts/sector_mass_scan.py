...
def check_cors(domain):
    delay()
    code, headers, body, err = fetch(
        f"https://{domain}/wp-json/wp/v2/users", extra_headers={"Origin": "https://evil.com"}
    )
    ...
    return {
        "acl": acl,
        "creds": creds,
        "reflected": acl == "https://evil.com",
        "credentialed": acl == "https://evil.com" and creds.lower() == "true",
        "wildcard": acl == "*",
    }
...
def check_leaks(domain):
    ...
    for path, keywords in checks.items():
        ...
        if code in (200, 301, 302) and body and len(body) > 30:
            ...
            if bs.strip().startswith(("<html", "<!doctype", "<script")) and path not in (
                "/robots.txt",
                "/sitemap.xml",
            ):
                continue
...
def save_report(domain, company, sector, r):
    ...
    lines = [
        ...
        f"| HTTP Code | {r.get('http_code', 'N/A')} |",
    ...
    if r.get("wp_users"):
        ...
        lines.append("| WP Users Exposed | 0 |")
    ...
    if r.get("xmlrpc_multicall"):
        ...
        lines.append(
            "**system.multicall enabled!** Attack surface: brute force, pingback SSRF, DDoS."
        )
        lines.append("")
...
def scan_domain(domain, company, sector):
    ...
    is_wp, signals = check_wp(body, headers)
    r["wp_signals"] = signals
    print(f"  [{'OK' if is_wp else 'NO'}] WP: {signals if is_wp else 'No'}")

    wp_login = check_wp_login(domain)
    r["wp_login"] = wp_login
        ...
        if wjc == 200:
            try:
                if isinstance(json.loads(wjb), dict):
                    r["is_wp"] = True
                    r["wp_signals"].append("wp-json")
                    print("  [OK] /wp-json/ detected")
            except:
                pass
    ...
    if r["is_wp"]:
        ...
        r["wp_users"] = users
        if users:
            print(f"  [VULN] {len(users)} users: {[u['name'] for u in users]}")
        else:
            print(f"  [INFO] /users: HTTP {uc}")

        cors_res = check_cors(domain)
        r["cors"] = cors_res
        status = (
            "CRITICAL+CREDS"
            if cors_res.get("credentialed")
            else (
                "REFLECTED"
                if cors_res.get("reflected")
                else ("WILDCARD" if cors_res.get("wildcard") else "SECURE")
            )
        )
        print(f"  [{'VULN' if cors_res.get('credentialed') else 'OK'}] CORS: {status}")

        xc, xcnt, xm, xmethods = check_xmlrpc(domain)
        r["xmlrpc_status"] = xc == 200
        r["xmlrpc_count"] = xcnt
        r["xmlrpc_multicall"] = xm
        r["sample_methods"] = xmethods
        if xc == 200:
        ...
        r["leaks"] = leaks
        if leaks:
            print(f"  [LEAK] {len(leaks)} leaks: {list(leaks.keys())}")
    else:
        leaks = check_leaks(domain)
        r["leaks"] = leaks
        if leaks:
            print(f"  [LEAK] {len(leaks)} leaks on non-WP: {list(leaks.keys())}")
...
if __name__ == "__main__":
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=5) as ex:
        ...
        for f in concurrent.futures.as_completed(fm):
            try:
                ...
                results.append({"domain": fm[f], "error": str(e), "alive": False})
    ...
    vuln = [
        r
        for r in wp
        if r.get("cors", {}).get("credentialed")
        or len(r.get("wp_users", [])) > 0
        or r.get("xmlrpc_multicall")
        or r.get("leaks")
    ]
    ...
    if vuln:
        ...
        for r in vuln:
            issues = []
            if r.get("cors", {}).get("credentialed"):
                issues.append("CORS+CREDS")
            if r.get("wp_users"):
                issues.append(f"USERS({len(r['wp_users'])})")
            if r.get("xmlrpc_multicall"):
                issues.append("MULTICALL")
            if r.get("leaks"):
                issues.append(f"LEAKS({len(r['leaks'])})")
