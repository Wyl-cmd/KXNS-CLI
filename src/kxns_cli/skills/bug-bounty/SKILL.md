...
> **For the full hunting methodology** — 5-phase non-linear workflow, developer psychology framework, session discipline, tool routing by phase, and Wide/Deep route selection — see below.

---

## ENGAGEMENT MODE CONFIRMATION (Before Anything Else)
...
|---|---|---|
| **Bug bounty** (H1 / Bugcrowd / Intigriti / private VDP) | Impact-demonstrated bugs ONLY. Full chain to attacker-attainable harm. | Hygiene (EoL software alone, permissive CSP alone, stack traces, info disclosure without concrete impact) |
| **Red team** (external client engagement) | Hygiene findings + recon + IoCs + defensive-state observations are ALL deliverables | Nothing — even "no finding here" is reportable |
...
**Hard rule:** Write the engagement type as the first line in your hunt notes. If you can't answer it from the user's instruction, ASK once. Don't assume.

---

## METHODOLOGY DISCIPLINE (False-Positive Prevention)

Most retracted findings come from recurring process bugs. Each has a hard rule.
...
- **NEVER** use `test`, `marker`, `evil`, `attacker`, `payload`, `javascript`, `script`, `AAAA`, `BBBB`, your domain name, or any string that could appear naturally in the target's HTML/JS/error messages.
- **Good markers:** `cpmark987abc`, `x4hd2k9pq`, a Collaborator subdomain prefix, or `__ZZ_MARKER_<random>_ZZ__`.
- Before claiming reflection: search the **baseline** (no-marker) response for the marker string. If it appears naturally, change your marker.
...
- Always diff the body: `diff <(curl ... baseline) <(curl ... bypass)`.
- Status-code-only claims ("Host header X gave 200 instead of 403") are the most common rejected-as-N/A category.
...
**Why:** zsh/bash array expansion fails silently on edge cases. A loop can produce zero iterations with no error if the array wasn't populated. Output looks complete but the test was skipped entirely.
...
### Pushback Protocol (When Asked to "Find More")

When the user disagrees with your stopping point:

**Default assumption: they are correct. You stopped early.**

Before pushing back:
1. **Re-read 3 more `hunt-*` skills** beyond what you have loaded.
2. **Re-attack the same surface** with the new skill checklists.
3. **Document negatives** as you go — a confirmed "no bug here" is itself a finding.
4. **Only after exhausting 3 new skills' checklists** do you push back, with a concrete list of what was tested.
