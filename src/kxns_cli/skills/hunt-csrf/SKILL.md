...
When JavaScript on the target origin constructs API URLs from user-controlled path segments, path traversal can be weaponized into same-origin CSRF — bypassing SameSite restrictions entirely:
...
### SameSite Lax — Chrome 2-Minute Exception

Chrome has a 2-minute exception where Lax cookies ARE sent on cross-site POSTs if the cookie was just set (designed for OAuth flows). Race window: set cookie, immediately trigger CSRF within 2 minutes — this defeats SameSite=Lax protections for freshly-issued session cookies.

### CSRF Testing Checklist (Quick Reference)
