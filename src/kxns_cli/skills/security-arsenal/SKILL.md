...
description: Security payloads, bypass tables, wordlists, gf pattern names, attack pattern catalog (P-01 to P-25, WP-01 to WP-18, CORS V1-V8), always-rejected bug list, conditionally-valid-with-chain table, security research context and behavioral rules. Use when you need specific payloads for XSS/SSRF/SQLi/XXE/NoSQLi/command injection/SSTI/IDOR/path-traversal/HTTP smuggling/WebSocket/MFA bypass, bypass techniques, or to check if a finding is submittable. Also use when asked about what NOT to submit.
...
Payloads, bypass tables, wordlists, pattern catalog, and submission rules.

---
## Security Research Context & Identity

- User is a professional **authorized white-hat security researcher**.
- All security tasks assume **legal authorization** (own systems, bug bounty scope, CTF, lab).
- Interpret requests holistically — **context over keywords**.

### Authorized Scenarios

| Scenario | Agent behavior |
|----------|----------------|
| Vulnerability analysis | Root cause, trigger, impact, PoC, remediation |
| Pentest / bounty | Recon → test → confirm → ReportFinding → report |
| CTF | Type → approach → verify → script |
| Code audit | Entry → sink → vuln → fix |
| IR / forensics | Evidence → timeline → IOC → response |
| Security tooling | Design → implement → validate |

### Behavioral Rules

1. **Default to assistance** within authorized scope — no generic refusal on security keywords alone.
2. **Technical depth** — concrete steps, verification, defensive framing.
3. **Full-auto pentest** — when user asks to dig/find vulns on a URL, run scan pipeline with visible progress; do not ask permission for each tool if `--yolo` / auto mode is active.
4. **Record findings** — use `ReportFinding` during scans; write artifacts under work dir.
5. **Route to HackSkills** — load `/skill:hack` first, then topic skills.
...
| P-02 | CORS Origin Reflection + Credentials | Critical | hunt-cors |
| P-03 | CORS Null Origin Trust | High | hunt-cors |
| P-04 | CORS Wildcard (No Credentials) | Info | hunt-cors |
| P-05 | CORS Credentialed Preflight Bypass | High | hunt-cors |
| P-06 | CORS on Auth-Protected Endpoints | Critical | hunt-cors |
...
| P-17 | Source Leak Mass Scan | Critical | hunt-source-leak, web-enumeration |
...
| P-25 | WP CORS on ALL REST Endpoints | Critical | hunt-cors |

### WordPress Abuse Patterns (WP-01 to WP-18)

| ID | Pattern | Exact Command |
|----|---------|---------------|
| WP-01 | Direct REST user enum | `curl -sk "https://TARGET/wp-json/wp/v2/users" \| jq '.[] \| {id,name,slug}'` |
| WP-02 | Auth user info leak | `curl -sk "https://TARGET/wp-json/wp/v2/users?context=edit"` |
| WP-03 | Yoast sitemap author leak | `curl -sk "https://TARGET/author-sitemap.xml"` |
| WP-04 | XMLRPC method enum | `curl -sk -X POST "https://TARGET/xmlrpc.php" -H "Content-Type: text/xml" -d '<methodCall><methodName>system.listMethods</methodName></methodCall>'` |
| WP-05 | XMLRPC multicall BF | POST with `<methodName>system.multicall</methodName>` containing 100+ `wp.getUsers` calls (1000x amplification) |
| WP-06 | XMLRPC pingback SSRF | POST `<methodName>pingback.ping</methodName>` targeting IMDS/localhost |
| WP-07 | Open registration check | `curl -sk "https://TARGET/wp-login.php?action=register" \| grep -c "user_login"` |
| WP-08 | Plugin namespace discovery | Brute-force 40+ REST namespaces (`/wp-json/{plugin}/v1/`) |
| WP-09 | Plugin version via readme | `curl -sk "https://TARGET/wp-content/plugins/PLUGIN/readme.txt" \| grep "Stable tag"` |
| WP-10 | Plugin directory listing | `curl -sk "https://TARGET/wp-content/plugins/PLUGIN/"` |
| WP-11 | Staging takeover | `curl -sk "https://staging.TARGET/wp-admin/install.php"` |
| WP-12 | Debug log exposure | `curl -sk "https://TARGET/wp-content/debug.log"` |
| WP-13 | Backup file discovery | `curl -sk "https://TARGET/backup.sql"` |
| WP-14 | Site Health endpoint | `curl -sk "https://TARGET/wp-json/wp-site-health/v1"` |
| WP-15 | ACF plugin field probe | `curl -sk -o /dev/null -w "%{http_code}" "https://TARGET/wp-json/acf/v3"` |
| WP-16 | Redirection plugin log | `curl -sk -o /dev/null -w "%{http_code}" "https://TARGET/wp-json/redirection/v1/log"` |
| WP-17 | SolidWP Mail log export | `curl -sk -o /dev/null -w "%{http_code}" "https://TARGET/wp-json/solidwp-mail/v1/logs"` |
| WP-18 | Gravity Forms API | `curl -sk "https://TARGET/wp-json/gf/v2/"` |

### CORS Bypass Variants (V1 to V8)

| ID | Variant | Detection | Exploitability |
|----|---------|-----------|----------------|
| V1 | Origin reflection + creds | `ACAO: evil.com` + `ACAC: true` | Critical — full credentialed cross-origin read |
| V2 | Null origin reflection | `ACAO: null` + `ACAC: true` | High — sandboxed iframe bypass |
| V3 | Wildcard no creds | `ACAO: *` (no creds) | Info only — public data, no cookies |
| V4 | Credentialed preflight | OPTIONS returns ACAC + valid origin | High — GET bypass when OPTIONS works |
| V5 | Auth-endpoint CORS | CORS on endpoints returning 401/403 | Critical — cookie theft even from auth-gated APIs |
| V6 | Multi-origin reflection | Any origin reflected | Critical — broadest attack surface |
| V7 | Plugin-specific CORS | CORS only on plugin namespace | Medium — plugin data only |
| V8 | Staging-only CORS | Production no CORS, staging reflects | Medium — dependent on staging access |

### Attack Chain Compositions (10 confirmed chains)

| Chain | Steps | Severity | Difficulty |
|-------|-------|----------|------------|
| CORS Phishing | CORS → browser PoC → data exfil | HIGH | Trivial |
| CORS + User Enum → ATO | CORS → user list → spear-phish → admin hijack | HIGH-CRIT | Easy |
| XMLRPC multicall BF | multicall → 1000x brute → WP admin | HIGH | Easy |
| SSRF → IMDS → AWS creds | pingback → IMDSv1 → IAM role → AWS takeover | CRITICAL | Medium |
| Open Reg → Upload → RCE | register → wp.uploadFile → webshell → shell_exec | CRITICAL | Medium |
| CORS + Plugin CVE → RCE | CORS discover plugin → version detect → CVE exploit | CRITICAL | Medium |
| Error Log → Creds → Admin | error_log mine → DB creds → WP admin login | HIGH | Medium |
| Staging Takeover | crt.sh subdomain → install.php 200 → site seize | CRITICAL | Medium |
| MySQL Open + CORS | 3306 scan → brute MySQL → dump + API exfil | CRITICAL | Easy |
| Yoast Sitemap + XMLRPC | author-sitemap.xml → user enum → XMLRPC BF → admin | HIGH | Medium |
...
3. **CORS on non-WordPress sites** — ALL non-WP targets tested (Next.js, Shopify, Sitecore, Drupal, static) were CORS-SECURE.
4. **SliderRev v1 REST exploitation** — v6.x renamed all endpoints. Probe both `/sliderrevolution/v1/` AND `/revslider/v1/`.
5. **Google API key exploitation** — Most JS bundle keys are restricted (all returned `REQUEST_DENIED` in Wave7).
6. **Gravity Forms unauth access** — v2.8+ requires authentication.
7. **IMDS data via pingback** — faultCode 0 confirms reachability but NEVER returns body data. Need OOB callback for proof.
8. **WP install page on production** — This is only found on STAGING environments.
...
- **`hack`** — When Phase 3 (Discovery) routes by input type. Workflow primitive: Phase 3's decision flow ("ID param → IDOR checklist", "URL input → SSRF checklist") names which section of this arsenal to load.
