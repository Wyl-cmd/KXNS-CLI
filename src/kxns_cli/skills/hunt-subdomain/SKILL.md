---
name: hunt-subdomain
description: "Complete subdomain hunting — enumeration (crt.sh, subfinder, DNS brute force, permutation, TLD expansion, CT monitoring), staging/dev discovery (WordPress install takeover, security gap analysis, internal subdomain leaks via crt.sh), and takeover exploitation (Azure cloudapp, Zendesk, Vercel, Fastly, S3, GitHub/GitLab Pages) with 5 ATO chains (OAuth redirect_uri, cookie-domain wildcard, CSP script-src, CORS regex, email DNS). Use for any subdomain recon, staging hunt, or takeover attack."
sources: github, hackerone_public, binarysecurity_research, can-i-take-over-xyz_research, field_recon
report_count: 3
---

## Phase 0 — Subdomain Enumeration

Comprehensive subdomain discovery using certificate transparency logs (crt.sh), DNS brute force, and passive sources. The first step in any recon pipeline — you can't attack what you don't know exists.

### Quick Enumeration Pipeline
...
# Common permutation patterns: %s-dev, dev-%s, %s-staging, %s-admin, %s-api, %s-test

# Resolve permutations
puredns resolve subs_permuted.txt -r resolvers.txt -o subs_permuted_alive.txt

# TLD expansion — company that owns target.com often neglects target.io/.net/.xyz
ROOT=$(echo "target.com" | cut -d. -f1)
while read tld; do echo "$ROOT.$tld"; done < tlds-alpha-by-domain.txt | httpx -silent -mc 200 > tlds_alive.txt
```
...
## Phase 1 — Staging & Dev Subdomain Hunt

Staging environments consistently have weaker security than production — no WAF, debug enabled, install pages accessible. Certificate transparency is the #1 source for discovering them.

### WordPress Install Page Takeover (Staging)
...
for sub in $(grep -iE 'staging|stage|dev' alive_subs.txt | awk '{print $1}' | head -10); do
  ...
  if [[ "$install_code" == "200" ]]; then
    echo "  [TAKEOVER] /wp-admin/install.php — fresh WP, can configure site!"
  ...
  if [[ "$config_code" == "200" || "$config_code" == "409" ]]; then
    echo "  [INFO] setup-config.php accessible — wp-config missing"
  fi

  # Check for exposed info.php / .env on staging
  for path in info.php phpinfo.php .env .env.local; do
    code=$(curl -sk -o /dev/null -w "%{http_code}" --max-time 5 "$sub/$path")
    [[ "$code" == "200" ]] && echo "  [CRITICAL] /$path exposed on staging!"
  done
done
```

### Production vs Staging Security Gap Analysis

```bash
DOMAIN="$1"
PROD="https://$DOMAIN"
STAGING=$(grep -iE 'staging|stage' alive_subs.txt | head -1 | awk '{print $1}')
...
### Quick Reference — Staging Indicators

| Indicator | What It Means | Action |
|-----------|---------------|--------|
| `/wp-admin/install.php` returns 200 | Fresh WordPress — no site configured | Install takeover |
| `/wp-admin/setup-config.php` returns 200/409 | wp-config missing | DB takeover via install flow |
| `info.php` / `phpinfo.php` on staging | Debug enabled | PHPInfo analysis |
| Staging has no Cloudflare/WAF | Direct origin access | Run full recon on origin IP |
...
## Phase 2 — Subdomain Takeover Hunting
