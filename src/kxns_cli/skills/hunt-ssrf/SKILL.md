...
## Advanced Technical Reference (from ssrf-playbook)

### IMDSv2 Approach

IMDSv2 requires a token retrieved via PUT, but if the SSRF vector supports custom headers, it's bypassable:
...
If the SSRF parameter or library allows custom headers (e.g., `requests.get(url, headers={...})` exposed via user input), full IMDSv2 bypass is possible.

### Kubernetes Service Account (file://)
...
Privileged container escape to host via SSRF:
```
...
When `gopher://` is blocked but `dict://` is allowed:
...
```
http://127.0.0.1:9000/actuator          ← Spring Boot actuator
...
### Application-Layer SSRF via XML

When the app parses user-supplied XML — XXE-style SSRF vector:
