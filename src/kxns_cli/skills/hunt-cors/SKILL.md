...
### High-Value Misconfiguration Checks
...
- **`cors-chain-automation`** — Multi-endpoint CORS credential test automation with 8-variant catalog (V1: Origin Reflection + Credentials, V2: Null Origin, V3: Wildcard No Creds, V4: Credentialed Preflight OPTIONS-only, V5: Auth-Required Endpoint Leak, V6: Multi-Origin Reflection, V7: Plugin-Specific CORS, V8: Staging-Only CORS). Automates endpoint discovery, regex bypass classification, browser PoC generation, and bulk cross-referencing with subdomain takeover. Chain primitive: `cors-regex-classifier.sh` identifies exact regex flaw → `cors-poc-generator.sh` produces ready-to-host HTML → `cors-bulk-chainer.sh` finds CORS+subdomain takeover overlaps.
- **`cors-credential-wordpress`** — WordPress REST API CORS credential reflection with full attack-chain construction. Documents CORS→User Enumeration→Spear-Phish→ATO and CORS→Cookie Theft→API Access→ATO chains. Multi-endpoint CORS matrix covers `/wp-json/wp/v2/users|posts|pages|media|comments|wc/v3/products|orders|gf/v2/forms|elementor/v1/globals`. Includes browser PoC templates for credentialled cross-origin data exfiltration and SameSite cookie considerations. Chain primitive: CORS on `/wp-json/wp/v2/users` exfils all WP users with slugs → craft spear-phish to admin → CORS phishing page steals WP session cookie → full site compromise.
