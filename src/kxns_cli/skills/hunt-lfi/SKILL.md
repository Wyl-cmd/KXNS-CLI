...
## Advanced Technical Reference (from path-traversal-lfi-playbook)
...
### wrapwrap — Prefix/Suffix Injection via Filter Chains

Tool: `ambionics/wrapwrap`. Adds arbitrary prefix and suffix to file content via php://filter chains. Useful for converting a file-read into XXE, SSRF, or deserialization trigger when the target parser requires specific wrapping.
...
..\..\windows\win<<    # Fuzzy match for win.ini
```

### /proc/self/fd Bruteforce

When file upload exists but path is unknown — uploaded files get temporary fd in `/proc/self/fd/`. Brute-force fd numbers 0–255 and include the temp file before cleanup.

### phpinfo() Assisted LFI (Race Condition)

1. POST multipart file to phpinfo() page → reveals tmp_name (`/tmp/phpXXXXXX`)
2. Include the temp file before PHP cleans it up
3. Requires many concurrent requests (race window ~10ms)

### iconv CVE-2024-2961

glibc < 2.39 iconv buffer overflow in PHP filter chains. Tool: `cfreal/cnext-exploits`. Converts LFI to RCE without needing writable paths or log poisoning. Uses `php://filter/convert.iconv.UTF-8.ISO-2022-CN-EXT/resource=` chain to trigger heap overflow → RCE.
