...
## Phase 9 — Parallel Mass Source Leak Scan

For batch scanning domains at scale with content verification (previously `source-leak-hunt`).
...
PATHS=(
  ".env" ".git/config" "wp-config.php.bak" "debug.log" "backup.sql"
  "info.php" "phpinfo.php" ".env.backup" ".env.local" ".env.production"
  "wp-config.php~" ".git/HEAD" "docker-compose.yml" "Dockerfile"
  ".DS_Store" "robots.txt" "sitemap.xml" "error_log" "wp-content/debug.log"
)
...
scan_target() {
  ...
  for path in "${PATHS[@]}"; do
    local code=$(curl -sk -o /tmp/leak_check_$$.tmp -w "%{http_code}" --max-time 5 "${url}/${path}" 2>/dev/null)
    if [[ "$code" == "200" ]]; then
      local content=$(head -c 2000 /tmp/leak_check_$$.tmp 2>/dev/null)
      local pattern="${PATTERNS[$path]}"
      if [[ -n "$pattern" ]] && echo "$content" | grep -qiE "$pattern"; then
        echo "[LEAK] ${url}/${path} (VERIFIED)"
        echo "${url}/${path}" >> "$OUTDIR/${domain}_leaks.txt"
      fi
    fi
  done
...
export -f scan_target; export OUTDIR; export PATHS
cat "$URLS_FILE" | xargs -P 30 -I {} bash -c 'scan_target "{}"'
```

### Parked Domain Detection

Parked/for-sale domains return HTTP 200 for every path with identical landing page HTML:
```bash
HASH1=$(curl -sk "https://$TARGET/robots.txt" | md5sum | cut -d' ' -f1)
HASH2=$(curl -sk "https://$TARGET/.env" | md5sum | cut -d' ' -f1)
[[ "$HASH1" == "$HASH2" ]] && echo "[SKIP] Parked domain — all paths return same content"
```

---

## Phase 10 — Backup File & Google Services Discovery

```bash
# bfac — multi-level backup file detection
bfac --url https://target.com --detection-technique all --level 3 --exclude-status-codes 404,500
...
waybackurls https://target.com | grep -iE \
  ...
  | sort -u | httpx -silent -mc 200 -o accessible_sensitive.txt
```

### Google Services Dorking
...
- hunt-cors — CORS testing on exposed API endpoints from source maps
- error-log-mining — PHP error log deep analysis
- js-secrets-extraction — client-side secret scanning in JS bundles
- phpinfo-to-rce — escalate phpinfo() exposure to RCE
