...
## Advanced Technical Reference (from xxe-playbook)

### XXE File Read Targets
...
Beyond standard `file://` and `http://`, try these alternative URI schemes:
...
### Local DTD Injection (Blind XXE Amplification)

When external entities are blocked by firewall/WAF/egress filter but `file://` to local files is still allowed, override entities inside a trusted local DTD to inject attacker-controlled definitions:
