...
pip install wsrepl                   # Python WS client with plugin support for cookie/token lifecycle
# ws-harness: bridge WS messages to HTTP for sqlmap/ffuf — python ws-harness.py -u "ws://host/path"
# Burp Suite Pro: WebSockets history (intercept/edit/replay), SocketSleuth (inspect/manipulate),
#   WebSocket Turbo Intruder (high-rate fuzzing), HTTP Request
