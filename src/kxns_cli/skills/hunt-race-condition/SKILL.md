...
### Multi-Step Exploitation (Pipeline Racing)

Workflow: `create → pay → confirm`. If **confirm** does not cryptographically bind to **pay** completion, race the pipeline:
...
**Success signal**: item marked paid/shipped without matching payment, or state skips backward. Also test multi-endpoint races: one `POST` to a state-change endpoint plus many `GET`s to a read endpoint released together to widen the TOCTOU observation window.

### External Tools

| Tool | Role |
|------|------|
| [JavanXD/Raceocat](https://github.com/JavanXD/Raceocat) | Race-focused HTTP client patterns |
| [nxenon/h2spacex](https://github.com/nxenon/h2spacex) | HTTP/2 low-level single-packet experimentation |
...
**Evidence checklist** — confirm all three before reporting:
