...
## Advanced Technical Reference (from open-redirect-playbook)

### Client-Side (JavaScript) Redirect Sinks
...
### Server-Side Redirect Sinks (Code Patterns)
...
### CSRF Referer Bypass via Open Redirect

Some CSRF protections validate that `Referer` header contains a trusted domain:
1. Attacker page links to `https://trusted.com/redirect?url=https://trusted.com/change-email`
2. Redirect from trusted.com preserves Referer from the trusted domain
3. CSRF check passes because `Referer = trusted.com`

### Tabnabbing / Reverse Tabnabbing

When `<a target="_blank">` opens an external page **without** `rel="noopener"`, the new page gains access to `window.opener` and can redirect the original tab:

```javascript
// On attacker-controlled page (opened via target="_blank"):
if (window.opener) {
    window.opener.location = "https://phishing.com/fake-login.html";
}
```

Detection pattern — vulnerable: `<a href="https://external.com" target="_blank">Click here</a>`. Safe: `<a href="..." target="_blank" rel="noopener noreferrer">`.

Where to look: user-generated content with links (forums, comments, profiles), PDF viewers/document previews opening in new tabs.

### Additional Bypass Techniques

| Validation | Bypass |
|---|---|
| Django `endswith('target.com')` | `http://evil.com/www.target.com` — path ends with target domain |
| Trusted-site double-redirect | `https://link.target.com/?url=http://evil.com` — chain through link service |
| Tab/newline in URL | `java%09script:alert(1)` — browser ignores whitespace |
| Null byte truncation | `http://trusted.com%00@evil.com` |
