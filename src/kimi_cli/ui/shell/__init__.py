...
class Shell:
    ...
    async def run_soul_command(self, user_input: str | list[ContentPart]) -> bool:
        ...
        try:
            ...
            console.print('[red]LLM not set, use "kxns api add" to configure[/red]')
    ...
    async def _auto_update(self) -> None:
        ...
        if result == UpdateResult.UPDATE_AVAILABLE:
            while True:
                toast(
                    "new version found, run `uv tool upgrade kxns-cli` to upgrade",
                    topic="update",
                    duration=30.0,
                )
...
def _print_welcome_info(name: str, info_items: list[WelcomeInfoItem]) -> None:
    head = Text.from_markup("[bold bright_red]Welcome to KXNS CLI![/bold bright_red]")
    help_text = Text.from_markup("[bright_red]Send /help for help information.[/bright_red]")
        ...
        if semver_tuple(latest_version) > semver_tuple(current_version):
            rows.append(
                Text.from_markup(
                    ...
                    "Please run `uv tool upgrade kxns-cli` to upgrade.[/yellow]"
                )
            )
