...
def help(app: Shell, args: str):
    ...
    renderables.append(
        BulletColumns(
            Group(
                Text.from_markup("[red]需要帮助吗？我可以帮你！[/red]"),
                Text.from_markup("[red]KXNS CLI 随时准备为您提供帮助！[/red]"),
                Text.from_markup("[red]发送消息给我，我会尽力协助您完成任务！[/red]"),
            ),
            bullet_style="red",
        )
    )
...
async def model(app: Shell, args: str):
    ...
    if not config.models:
        console.print('[yellow]No models configured, use "kxns api add" to add a model.[/yellow]')
        return

    if not config.is_from_default_location:
        console.print(
            "[yellow]Model switching requires default config file; "
    ...
    for name in sorted(config.models):
        ...
        label = f"{model_cfg.model}{marker}"
...
def feedback(app: Shell, args: str):
    """Submit feedback to make KXNS CLI better"""
    import webbrowser
