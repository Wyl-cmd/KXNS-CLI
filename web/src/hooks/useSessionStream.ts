 ...
 *
 * 2) Browser tabs (two Kxns Hunter pages open in Chrome, etc.):
 *
...
type UseSessionStreamOptions = {
  ...
  /** Initial thinking mode state (from global config) */
...
type UseSessionStreamReturn = {
  ...
  thinking: boolean;
  /** Set thinking mode via silent RPC (no context message) */
  sendSetThinking: (enabled: boolean) => void;
  /** Current session model name (null = use global default) */
  sessionModel: string | null;
  /** Set session model via silent RPC */
  sendSetModel: (modelName: string) => void;
  /** Available slash commands from the server */
  slashCommands: SlashCommandDef[];
  /** Mark the next disconnect as intentional (e.g., before session delete) */
...
export function useSessionStream(
  ...
  const intentionalDisconnectRef = useRef(false); // Track intentional disconnect (e.g., session delete)
  ...
  }, [resetStepState, setAwaitingFirstResponse, initialThinking]);
      ...
      switch (event.type) {
        ...
        case "ToolResult": {
          ...
          if (Array.isArray(return_value.output)) {
            ...
            // For non-browser-renderable URLs (e.g. ms:// from Kxns Hunter model),
        ...
        case "StatusUpdate": {
          ...
          const nextThinking = event.payload.thinking;
          if (typeof nextThinking === "boolean") {
            setThinking(nextThinking);
          }

          const nextModel = event.payload.model;
    ...
    (data: string) => {
      try {
        ...
        if (message.error) {
          ...
          const isNonFatalRpcError =
            message.id &&
            statusRef.current !== "streaming" &&
            statusRef.current !== "submitted";

          if (isNonFatalRpcError) {
            console.warn("[SessionStream] Non-fatal RPC error:", message.error);
            return;
          }
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      ws.onclose = (event) => {
        ...
        // Handle specific close codes (only show errors if not intentional disconnect)
        if (!wasIntentional) {
          if (event.code === 4004) {
  ...
  const cancel = useCallback(() => {
    ...
    try {
      ...
      if (shouldAwaitIdle) {
        setTimeout(() => {
          if (statusRef.current === "submitted" && awaitingIdleRef.current) {
            console.warn("[SessionStream] Cancel timeout, resetting to ready");
            awaitingIdleRef.current = false;
            setStatus("ready");
          }
        }, 30000);
    ...
    async (text: string) => {
      ...
      if (!wsRef.current || wsRef.current.readyState !== WebSocket.OPEN) {
        ...
        if (pendingMessageRef.current) {
          console.warn("[SessionStream] Overwriting pending message, previous message will be lost");
        }
  ...
  const sendSetPlanMode = useCallback((enabled: boolean) => {
    const ws = wsRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      toast.error("Cannot change plan mode", {
        description: "Not connected to session. Please wait for connection.",
      });
      return;
    }
    ...
    try {
      ...
      toast.error("Failed to change plan mode", {
  ...
  // Set thinking mode via silent RPC (no context message)
  const sendSetThinking = useCallback((enabled: boolean) => {
    const ws = wsRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      toast.error("Cannot change thinking mode", {
        description: "Not connected to session. Please wait for connection.",
      });
      return;
    }
  ...
  // Sync thinking state when global defaultThinking changes
  useEffect(() => {
    setThinking(initialThinking);
    const ws = wsRef.current;
    if (ws && ws.readyState === WebSocket.OPEN) {
      const message: JsonRpcRequest = {
        ...
        method: "set_thinking",
        id: uuidV4(),
        params: { enabled: initialThinking },
      };
      try {
        ws.send(JSON.stringify(message));
      } catch {}
  ...
  // Set session model via silent RPC
  const sendSetModel = useCallback((modelName: string) => {
    const ws = wsRef.current;
    if (!ws || ws.readyState !== WebSocket.OPEN) {
      toast.error("Cannot switch model", {
        description: "Not connected to session. Please wait for connection.",
      });
      return;
    }
