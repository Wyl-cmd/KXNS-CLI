...
type ChatWorkspaceContainerProps = {
  ...
  onMarkIntentionalDisconnect?: (fn: () => void) => void;
};
...
export function ChatWorkspaceContainer({
  ...
  const sessionStream = useSessionStream({
    ...
    initialThinking: config?.defaultThinking ?? false,
  });
  ...
  useEffect(() => {
    onMarkIntentionalDisconnect?.(markIntentionalDisconnect);
  }, [markIntentionalDisconnect, onMarkIntentionalDisconnect]);
  ...
  const handlePromptSubmit = useCallback(
    async (message: PromptInputMessage) => {
      ...
      const messageText =
        message.text.trim() ||
        (message.files.length > 0 ? "KXNS_FILE_UPLOAD_WITHOUT_MESSAGE" : "");
  ...
  const handleThinkingChange = useCallback((enabled: boolean) => {
    sendSetThinking(enabled);
  }, [sendSetThinking]);

  const handleSessionModelChange = useCallback((modelName: string) => {
    sendSetModel(modelName);
  }, [sendSetModel]);
