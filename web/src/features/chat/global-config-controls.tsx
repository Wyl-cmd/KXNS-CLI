...
export function GlobalConfigControls({
  ...
  const activeModel = sessionModel ?? config?.defaultModel ?? null;

  const currentModel = useMemo(() => {
    if (!config) {
      return null;
    }
    return config.models.find((m) => m.name === activeModel) ?? null;
  }, [config, activeModel]);

  const thinkingState = useMemo(
    () => getThinkingState(currentModel, config?.defaultThinking ?? false),
    [currentModel, config?.defaultThinking],
  );

  const thinkingDisabled =
    isLoading || isUpdating || thinkingState === "disabled" || thinkingState === "forced";
  const handleSelectModel = useCallback(
    ...
    [config, update, activeModel, onSessionModelChange],
  );
        ...
        <>
          ...
          <Tooltip>
            <TooltipTrigger asChild>
              <div className="flex h-9 items-center gap-2 rounded-md px-2 cursor-pointer">
